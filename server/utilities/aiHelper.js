...
Generate a concise session description with these sections:
- Overview (1-2 sentences)
- Learning Outcomes (2-3 bullet points using "•")
- Requirements (1-2 bullet points using "•")
...
Keep response under 200 words.`,
  ...
  search: `You are a search assistant for SkillSwap, a peer-to-peer skill sharing platform.
Given a learner's natural language query, extract 3-5 relevant search keywords, skill names, or topics.
Respond with ONLY the keywords separated by commas. No labels, no extra text.`,

  chat: `You are SwapMind AI, a helpful guide for a peer-to-peer skill sharing platform.
...
function classifyError(raw, statusCode) {
  ...
  if (msg.includes("api_key") || msg.includes("api key") || msg.includes("api-key") || (msg.includes("not found") && msg.includes("key"))) {
    return { message: "AI service is not configured. Add GROQ_API_KEY to your .env file.", code: 503 };
  }
  if (msg.includes("safety") || msg.includes("content_filter") || msg.includes("blocked")) {
    return { message: "Content filtered by AI safety guidelines. Please rephrase.", code: 400 };
  }
  if (statusCode === 429 || msg.includes("quota") || msg.includes("rate_limit") || msg.includes("rate limit") || msg.includes("too many requests")) {
    return { message: "AI service is currently overloaded. Please try again.", code: 429 };
  }
  if (statusCode === 503 || msg.includes("unavailable") || msg.includes("overloaded") || msg.includes("down") || msg.includes("timed out")) {
...
async function generateContent(systemKey, userMessage, options = {}) {
  ...
  const messages = buildPrompt(systemKey, validated);

  if (!API_KEY) {
    throw Object.assign(
      new Error("AI service is not configured. Add GROQ_API_KEY to your .env file."),
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (!text) {
        console.log("AI empty response:", JSON.stringify(completion, null, 2));
        throw Object.assign(
  ...
  if (!lastError) {
    throw Object.assign(
      new Error("AI service is temporarily overloaded. Please try again later."),
      { statusCode: 503, expose: true }
    );
  }

  const classified = classifyError(lastError.message || "", lastError.status || lastError.statusCode || 500);
  if (classified) {
    throw Object.assign(
...
module.exports = {
  ...
  searchSessions: (prompt) => generateContent("search", prompt, { maxLength: 500, temperature: 0.3 }),
};
