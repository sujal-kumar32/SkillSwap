...
// ─────────────────────────────────────────────────────────────────────────────
// useChatSocket — manages socket event listeners and typing state
// ─────────────────────────────────────────────────────────────────────────────

const useChatSocket = ({ socket, activeChat, setActiveChat, loadConversations, user }) => {
  ...
  const applyReaction = (prev, messageId, reactions) => {
    if (!prev) return prev;
    const updated = prev.messages.map((msg) => {
  ...
  const applyDeletion = (prev, messageId) => {
    if (!prev) return prev;
    const updated = prev.messages.map((m) =>
      m._id === messageId ? { ...m, isDeleted: true, message: "", attachments: [], reactions: [] } : m
    );
    return { ...prev, messages: updated };
  };
  ...
  useEffect(() => {
    ...
    const onReaction = (data) => {
      if (activeChat && data.chatId === activeChat._id) {
        setActiveChat((prev) => applyReaction(prev, data.messageId, data.reactions));
      }
    };

    const onMessageDeleted = (data) => {
      if (activeChat && data.chatId === activeChat._id) {
        setActiveChat((prev) => applyDeletion(prev, data.messageId));
      }
    };
  ...
  }, [socket, activeChat, loadConversations, user, setActiveChat]);
...
const ConversationSidebar = ({
  conversations, convLoading, searchQuery, filteredConvs, searchMsgResults,
  chatId, onSelectChat, onSearch, onShowCompose, onShowBlocked, isOnline, user,
  sidebarVisible,
}) => (
  <div className={`d-flex flex-column ${sidebarVisible ? "d-flex" : "d-none d-md-flex"}`}
    ...
    <div style={{ padding: "16px 20px", borderBottom: "1px solid #eef2f7" }}>
      <div className="d-flex justify-content-between align-items-center mb-2">
        ...
        <button onClick={onShowCompose} className="btn btn-sm btn-primary rounded-pill px-3 fw-semibold" style={{ fontSize: "0.75rem" }}>
    ...
    <div style={{ flex: 1, overflow: "auto" }}>
          ...
          {filteredConvs.length > 0 && (
            <>
              ...
              {filteredConvs.map((c) => {
                ...
                return (
                  <div key={c._id} onClick={() => onSelectChat(c._id)}
                    style={{
          ...
          {searchMsgResults && searchMsgResults.length > 0 && (
            <>
              ...
              {searchMsgResults.map((r) => (
                <div key={r.messageId} onClick={() => onSelectChat(r.chatId)}
      ...
      <div style={{ padding: "12px 20px", borderTop: "1px solid #eef2f7" }}>
        <button onClick={onShowBlocked} style={{ background: "none", border: "none", fontSize: "0.72rem", color: "#94a3b8", cursor: "pointer", padding: 0 }}>
...
const BlockedUsersModal = ({ show, users, onClose, onUnblock }) => {
  if (!show) return null;
  return (
          ...
          {users.length === 0 ? (
            ...
            users.map((u) => (
              <div key={u._id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 20px" }}>
                ...
                <button onClick={() => onUnblock(u._id)} className="btn btn-sm btn-outline-primary rounded-pill fw-semibold" style={{ fontSize: "0.7rem" }}>Unblock</button>
              </div>
            ))
          )}
        </div>
...
const ComposeModal = ({ show, query, results, onClose, onSearch, onStartChat }) => {
  if (!show) return null;
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1050, display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: "10vh" }}>
      <div onClick={onClose} style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.4)" }} />
      <div style={{ position: "relative", width: 400, maxWidth: "90vw", background: "#fff", borderRadius: 16, boxShadow: "0 20px 60px rgba(0,0,0,0.3)", overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid #eef2f7" }}>
          ...
          <div className="input-group input-group-sm mt-2">
            <span className="input-group-text bg-white" style={{ borderRight: "none", borderColor: "#e2e8f0" }}><i className="fa fa-search" style={{ fontSize: "0.7rem", color: "#94a3b8" }} /></span>
            <input className="form-control" placeholder="Search users..." value={query} autoFocus
              onChange={(e) => onSearch(e.target.value)}
        ...
        <div style={{ maxHeight: 320, overflow: "auto" }}>
          {query.trim() && results.length === 0 ? (
            ...
            results.map((u) => (
              <div key={u._id} onClick={() => onStartChat(u._id)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 20px", cursor: "pointer", transition: "background 0.15s" }}
                onMouseEnter={(e) => e.currentTarget.style.background = "#f8fafc"}
                onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                <img src={u.profileImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(u.name || "?")}&background=0d6efd&color=fff&size=36`}
                  alt="" style={{ width: 36, height: 36, borderRadius: "50%", objectFit: "cover" }} />
...
const ReactionStrip = ({ reactCounts, isMe }) => {
  const entries = Object.entries(reactCounts);
...
const MessageBubble = ({ msg, isMe, showDate, onDoubleClick, onDeleteMessage }) => {
  ...
  return (
    <>
      ...
      {hasReactions && !isDeleted && <ReactionStrip reactCounts={reactCounts} isMe={isMe} />}
    </>
  );
};
...
const MessageList = ({ activeChat, user, typingUser, messagesEndRef, onDoubleClick, onDeleteMessage }) => {
  const messages = activeChat?.messages || [];
  const typingName = typingUser && activeChat
    ? (activeChat.participants.find((p) => p._id === typingUser)?.name || "Someone")
    : "";
  ...
  return (
    <div style={{ flex: 1, overflow: "auto", padding: "16px 20px", background: "#fafbfc" }}>
      {messages.map((msg, i) => {
        ...
        return (
          <MessageBubble key={msg._id || i} msg={msg} isMe={isMe} showDate={showDate}
            onDoubleClick={onDoubleClick} onDeleteMessage={onDeleteMessage} />
        );
      })}
      {typingUser && (
...
const ChatHeader = ({ activeChat, chatPartner, isOnline, onBack, onBlockToggle }) => (
  <div style={{ padding: "12px 20px", borderBottom: "1px solid #eef2f7", background: "#fff", display: "flex", alignItems: "center", gap: 12 }}>
    <button className="btn btn-sm d-md-none" onClick={onBack} style={{ border: "none", background: "none" }}>
      <i className="fa fa-arrow-left" />
    </button>
    <div style={{ position: "relative", flexShrink: 0 }}>
      <img src={chatPartner?.profileImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(chatPartner?.name || "?")}&background=0d6efd&color=fff&size=36`}
        alt="" style={{ width: 36, height: 36, borderRadius: "50%", objectFit: "cover" }} />
      {isOnline(chatPartner?._id) && <span style={{ width: 10, height: 10, borderRadius: "50%", background: "#22c55e", border: "2px solid #fff", position: "absolute", bottom: 0, right: 0 }} />}
    </div>
    <div style={{ flex: 1 }}>
      <Link to={`/profile/${chatPartner?._id}`} style={{ textDecoration: "none", color: "inherit" }}>
        <span className="fw-semibold" style={{ fontSize: "0.9rem" }}>{chatPartner?.name || "Unknown"}</span>
      </Link>
      {isOnline(chatPartner?._id) && <div style={{ fontSize: "0.65rem", color: "#22c55e", marginTop: -1 }}>Online</div>}
    </div>
    {!activeChat.theyBlockedMe && (
      <button onClick={onBlockToggle} title={activeChat.blocked ? "Unblock user" : "Block user"}
        style={{ background: "none", border: "none", color: activeChat.blocked ? "#dc2626" : "#94a3b8", cursor: "pointer", padding: "6px 8px", fontSize: "1rem" }}>
...
const ChatInput = ({
  activeChat, attachments, uploading, input, sending,
  onInputChange, onKeyDown, onSendMessage, onFileSelect, onRemoveAttachment,
  fileInputRef, inputRef,
}) => {
  if (activeChat.blocked || activeChat.theyBlockedMe) {
    return (
  ...
  return (
    <div style={{ padding: "12px 20px", borderTop: "1px solid #eef2f7", background: "#fff" }}>
        ...
        <div className="d-flex" style={{ gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
          {attachments.map((att, i) => (
            <div key={i} style={{ position: "relative", width: 60, height: 60, borderRadius: 8, overflow: "hidden", border: "1px solid #e2e8f0" }}>
              ...
              <button onClick={() => onRemoveAttachment(i)} style={{ position: "absolute", top: 1, right: 1, width: 18, height: 18, borderRadius: "50%", background: "rgba(0,0,0,0.5)", color: "#fff", border: "none", fontSize: "0.55rem", display: "grid", placeItems: "center", lineHeight: 1 }}>×</button>
            </div>
          ))}
        </div>
      )}
      <div className="d-flex" style={{ gap: 8, alignItems: "flex-end" }}>
        ...
        <input ref={fileInputRef} type="file" accept="image/*,.pdf,.doc,.docx,.txt,.zip" onChange={onFileSelect} style={{ display: "none" }} />
        <textarea
          ref={inputRef}
        ...
        />
        <button className="btn btn-primary rounded-circle" onClick={onSendMessage} disabled={sending || (!input.trim() && attachments.length === 0)}
...
const ChatPanel = ({
  chatId, chatLoading, activeChat, showMobileList, onBack,
  typingUser, user, isOnline, messagesEndRef,
  onDoubleClick, onDeleteMessage, onBlockToggle,
  input, onInputChange, onKeyDown, onSendMessage, sending,
  uploading, attachments, onRemoveAttachment, onFileSelect,
  fileInputRef, inputRef,
}) => {
  if (!chatId) {
    return (
  ...
  const chatPartner = activeChat.participants
    ? activeChat.participants.find((p) => p._id !== user?._id) || activeChat.participants[0]
    : null;

  return (
    <div className={`d-flex flex-column flex-grow-1 ${showMobileList ? "d-none d-md-flex" : "d-flex"}`}>
      <ChatHeader activeChat={activeChat} chatPartner={chatPartner} isOnline={isOnline}
        onBack={onBack} onBlockToggle={onBlockToggle} showMobileList={showMobileList} />
      {activeChat.theyBlockedMe && (
      ...
      {!activeChat.theyBlockedMe && activeChat.blocked && (
        <div style={{ padding: "10px 20px", background: "#fef2f2", borderBottom: "1px solid #fecaca", textAlign: "center", fontSize: "0.8rem", color: "#dc2626" }}>
          <i className="fa fa-ban me-1" />You blocked this user. <button onClick={onBlockToggle} style={{ background: "none", border: "none", color: "#dc2626", textDecoration: "underline", cursor: "pointer", padding: 0 }}>Unblock</button>
        </div>
      )}
      <MessageList activeChat={activeChat} user={user} typingUser={typingUser}
        messagesEndRef={messagesEndRef} onDoubleClick={onDoubleClick} onDeleteMessage={onDeleteMessage} />
      <ChatInput activeChat={activeChat} attachments={attachments}
        uploading={uploading} input={input} sending={sending}
        onInputChange={onInputChange} onKeyDown={onKeyDown}
        onSendMessage={onSendMessage} onFileSelect={onFileSelect}
        onRemoveAttachment={onRemoveAttachment}
        fileInputRef={fileInputRef} inputRef={inputRef} />
    </div>
  );
};

// ─────────────────────────────────────────────────────────────────────────────
// MessagesPage (main entry)
// ─────────────────────────────────────────────────────────────────────────────
...
const MessagesPage = () => {
  ...
  const { typingUser } = useChatSocket({ socket, activeChat, setActiveChat, loadConversations, user });
  ...
  return (
    <>
      ...
      <div className="bg-image" style={{ height: "calc(100vh - 64px)", display: "flex", overflow: "hidden" }}>
        <ConversationSidebar conversations={conversations} convLoading={convLoading}
          searchQuery={searchQuery} filteredConvs={filteredConvs}
          searchMsgResults={searchMsgResults} chatId={chatId}
          onSelectChat={handleSelectChat} onSearch={handleSearch}
          onShowCompose={() => setShowCompose(true)} onShowBlocked={handleShowBlocked}
          isOnline={isOnline} user={user}
          sidebarVisible={showMobileList || !chatId} />
        <BlockedUsersModal show={showBlocked} users={blockedUsers}
          onClose={handleCloseBlocked} onUnblock={handleUnblockUser} />
        <ComposeModal show={showCompose} query={composeQuery} results={composeResults}
          onClose={handleCloseCompose} onSearch={handleComposeSearch}
          onStartChat={startChat} />
        <ChatPanel chatId={chatId} chatLoading={chatLoading} activeChat={activeChat}
          showMobileList={showMobileList} onBack={() => setShowMobileList(true)}
          typingUser={typingUser} user={user} isOnline={isOnline}
          messagesEndRef={messagesEndRef}
          onDoubleClick={handleDoubleClick} onDeleteMessage={handleDeleteMessage}
          onBlockToggle={handleBlockToggle}
          input={input} onInputChange={handleInputChange} onKeyDown={handleKeyDown}
          onSendMessage={sendMessage} sending={sending}
          uploading={uploading} attachments={attachments}
          onRemoveAttachment={removeAttachment} onFileSelect={handleFileSelect}
          fileInputRef={fileInputRef} inputRef={inputRef} />
      </div>
    </>
  );
